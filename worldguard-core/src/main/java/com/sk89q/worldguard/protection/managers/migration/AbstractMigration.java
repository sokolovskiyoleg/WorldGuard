        ...
        try {
            for (RegionDatabase store : driver.getAll()) {
                try {
                    ...
                    log.log(Level.WARNING, "Произошла ошибка при миграции одного мира (" + store.getName() + ")", e);
                }
            }

            postMigration();
        } catch (StorageException e) {
            throw new MigrationException("Миграция не удалась, потому что не удалось получить список всех миров для переноса.", e);
        }
    }
